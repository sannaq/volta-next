...
function liqHTML() {
  return '<style>'
    ...
    + '<div id="lqLines" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:14px;align-items:center"><span style="font-size:11px;color:#5a6576;margin-right:2px">분석선</span></div>'
    + '<div id="lqDeriv"></div>'
...
function makeLiqEngine(root, initSym) {
  ...
  function draw() {
    ...
    // === 분석선: 지지/저항(24h 고·저) · 회귀 채널(파랑 2선) · 골드 스윙 추세선 ===
    if (st.lineOn.sr && n >= 2) { var srHi = -Infinity, srLo = Infinity; for (var s1 = 0; s1 < n; s1++) { if (cs[s1].h > srHi) srHi = cs[s1].h; if (cs[s1].l < srLo) srLo = cs[s1].l; } ctx.setLineDash([6, 5]); ctx.lineWidth = 1; [[srHi, '246,70,93', '저항 ' + fmtPx(srHi)], [srLo, '46,189,133', '지지 ' + fmtPx(srLo)]].forEach(function (d) { if (d[0] < lo || d[0] > hi) return; var yy = y(d[0]); ctx.strokeStyle = 'rgba(' + d[1] + ',0.75)'; ctx.beginPath(); ctx.moveTo(cl, yy); ctx.lineTo(cr, yy); ctx.stroke(); ctx.fillStyle = 'rgba(' + d[1] + ',0.95)'; ctx.font = '700 10px sans-serif'; ctx.textAlign = 'left'; ctx.fillText(d[2], cl + 4, yy - 3); ctx.textAlign = 'start'; }); ctx.setLineDash([]); }
  ...
  function start() {
    ...
    buildPills(); buildLineToggles(); renderFeed(); resize(); [60, 200, 500, 1200, 2500].forEach(function (ms) { timers.push(setTimeout(resize, ms)); });
