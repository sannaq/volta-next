 ...
 * }
 */
const KEY = "volta_paper_wallet_v3";
const INITIAL = brand.paperCashUSDT;

function fresh() {
  return {
    cashUSDT: INITIAL,
    holdings: {},           // sym → 보유수량(롱 노출)
    borrowed: {},           // sym → 레버리지로 빌린 USDT(격리)
    entry: {},              // sym → 평균 진입가
    openOrders: [],
    history: [],
...
/** 가상 자산 스냅샷 (평가자산·수익률) — 레버리지 부채 반영 */
export function walletSummary(wallet, prices) {
  let posValue = 0, debt = 0;
  for (const s in wallet.holdings) posValue += wallet.holdings[s] * (prices[s]?.px || 0);
  for (const s in (wallet.borrowed || {})) debt += wallet.borrowed[s] || 0;
  const equity = wallet.cashUSDT + posValue - debt;
  const principal = wallet.principal || INITIAL;
  const pnl = equity - principal;
  const roi = principal > 0 ? (pnl / principal) * 100 : 0;
  return { equity, posValue, debt, principal, pnl, roi };
}
